...
RecursiveProgram : Container ℓ₁ ℓ₂ → (I : Set ℓ₃) → (I → Set ℓ₄) → Set (ℓ₁ ⊔ ℓ₂ ⊔ ℓ₃ ⊔ ℓ₄)
...
recursionHandler x n i = recursionHandler' x n (x i)
  ...
  recursionHandler' f (suc n) (impure (inj₁ (call i) , c)) = recursionHandler' f n (f i) >>= maybe (recursionHandler' f (suc n) ∘ c ∘ lift) (pure nothing)
  recursionHandler' {ℓ₂ = ℓ₂} {ℓ₄ = ℓ₄} f n (impure (inj₂ s , c)) = impure (s , recursionHandler' f n ∘ c ∘ lift)
