...
~true⇔false : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (x : Free ε α) → ~ true ⊢ x ⇔ false ⊢ x
to (~true⇔false _) h = ⊥-elim₀ (h tt)
from (~true⇔false _) h = ⊥-elim h

~false⇔true : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (x : Free ε α) → ~ false ⊢ x ⇔ true ⊢ x
to (~false⇔true _) _ = tt
from (~false⇔true _) _ h = ⊥-elim h

~~f⇔f : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (f : Formula ε) → (x : Free ε α) → ~ ~ f ⊢ x ⇔ f ⊢ x
to (~~f⇔f f x) ¬¬h with ⊢-dec f x
... | no ¬h = ⊥-elim₀ (¬¬h ¬h)
... | yes h = h
from (~~f⇔f _ _) h ¬h = ¬h h

~|f₁∧f₂|⇔~f₁∨~f₂ : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (f₁ f₂ : Formula ε) → (x : Free ε α) → ~ (f₁ ∧ f₂) ⊢ x ⇔ ~ f₁ ∨ ~ f₂ ⊢ x
...
f∧f⇔f : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (f : Formula ε) → (x : Free ε α) → f ∧ f ⊢ x ⇔ f ⊢ x
to (f∧f⇔f _ _) (h , _) = h
...
|f₁∧f₂|∧f₃⇔f₁∧|f₂∧f₃| : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (f₁ f₂ f₃ : Formula ε) → (x : Free ε α) → (f₁ ∧ f₂) ∧ f₃ ⊢ x ⇔ f₁ ∧ (f₂ ∧ f₃) ⊢ x
to (|f₁∧f₂|∧f₃⇔f₁∧|f₂∧f₃| _ _ _ _) ((h₁ , h₂) , h₃) = h₁ , h₂ , h₃
from (|f₁∧f₂|∧f₃⇔f₁∧|f₂∧f₃| _ _ _ _) (h₁ , h₂ , h₃) = (h₁ , h₂) , h₃

f₁∧|f₂∨f₃|⇔|f₁∧f₂|∨|f₁∧f₃| : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (f₁ f₂ f₃ : Formula ε) → (x : Free ε α) → f₁ ∧ (f₂ ∨ f₃) ⊢ x ⇔ (f₁ ∧ f₂) ∨ (f₁ ∧ f₃) ⊢ x
to (f₁∧|f₂∨f₃|⇔|f₁∧f₂|∨|f₁∧f₃| _ _ _ _) (h₁ , inj₁ h₂) = inj₁ (h₁ , h₂)
to (f₁∧|f₂∨f₃|⇔|f₁∧f₂|∨|f₁∧f₃| _ _ _ _) (h₁ , inj₂ h₃) = inj₂ (h₁ , h₃)
from (f₁∧|f₂∨f₃|⇔|f₁∧f₂|∨|f₁∧f₃| _ _ _ _) (inj₁ (h₁ , h₂)) = h₁ , inj₁ h₂
from (f₁∧|f₂∨f₃|⇔|f₁∧f₂|∨|f₁∧f₃| _ _ _ _) (inj₂ (h₁ , h₃)) = h₁ , inj₂ h₃

f∧true⇔f : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (f : Formula ε) → (x : Free ε α) → f ∧ true ⊢ x ⇔ f ⊢ x
to (f∧true⇔f _ _) (h , _) = h
from (f∧true⇔f _ _) h = h , tt

f∧false⇔false : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (f : Formula ε) → (x : Free ε α) → f ∧ false ⊢ x ⇔ false ⊢ x
...
f∨true⇔true : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (f : Formula ε) → (x : Free ε α) → f ∨ true ⊢ x ⇔ true ⊢ x
to (f∨true⇔true _ _) _ = tt
from (f∨true⇔true _ _) _ = inj₂ tt

f∨false⇔f : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (f : Formula ε) → (x : Free ε α) → f ∨ false ⊢ x ⇔ f ⊢ x
...
~|⟨c⟩f|⇔[c]~f : ⦃ eq : IsDecEquivalence {A = C ε} _≡_ ⦄ → (c : C ε) → (f : Formula ε) → (x : Free ε α) → ~ (⟨ c ⟩ f) ⊢ x ⇔ [ c ] ~ f ⊢ x
...
⟨c⟩false⇔false : ⦃ eq : IsDecEquivalence {A = C ε} _≡_ ⦄ → (c : C ε) → (x : Free ε α) → ⟨ c ⟩ false ⊢ x ⇔ false ⊢ x
to (⟨c⟩false⇔false c₁ (step c₂ _)) h∃ with c₁ ≟ c₂
... | no _ = ⊥-elim h∃
to (⟨c⟩false⇔false c₁ (step .c₁ _)) () | yes refl
from (⟨c⟩false⇔false _ _) ()

⟨c⟩f₁∨f₂⇔|⟨c⟩f₁|∨|⟨c⟩f₂| : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (c : C ε) → (f₁ f₂ : Formula ε) → (x : Free ε α) → ⟨ c ⟩ f₁ ∨ f₂ ⊢ x ⇔ (⟨ c ⟩ f₁) ∨ (⟨ c ⟩ f₂) ⊢ x
...
|⟨c⟩f₁|∧|[c]f₂|→⟨c⟩f₁∧f₂ : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (c : C ε) → (f₁ f₂ : Formula ε) → (x : Free ε α) → (⟨ c ⟩ f₁) ∧ ([ c ] f₂) ⊢ x → ⟨ c ⟩ f₁ ∧ f₂ ⊢ x
...
~|[c]f|⇔⟨c⟩~f : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (c : C ε) → (f : Formula ε) → (x : Free ε α) → ~ ([ c ] f) ⊢ x ⇔ ⟨ c ⟩ ~ f ⊢ x
...
[c]true⇔true : ⦃ _ : IsDecEquivalence {A = C ε} _≡_ ⦄ → (c : C ε) → (x : Free ε α) → [ c ] true ⊢ x ⇔ true ⊢ x
