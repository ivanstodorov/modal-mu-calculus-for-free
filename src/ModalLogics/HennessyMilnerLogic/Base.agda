...
_⊢_ : {ε : Effect {ℓ₁} {ℓ₂}} → ⦃ Eq (Effect.C ε) ⦄ → {α : Set ℓ₃} → Formula ε → Free ε α → Set ℓ₂
true ⊢ x = ⊤
false ⊢ x = ⊥
(~ f) ⊢ x = ¬ (f ⊢ x)
(f₁ ∧ f₂) ⊢ x = f₁ ⊢ x × f₂ ⊢ x
...
postulate
  ⊢-dec : {ε : Effect {ℓ₁} {ℓ₂}} → ⦃ _ : Eq (Effect.C ε) ⦄ → {α : Set ℓ₃} → (f : Formula ε) → (x : Free ε α) → Dec (f ⊢ x)

_⊢_!_ : {ε : Effect {ℓ₁} {ℓ₂}} → ⦃ Eq (Effect.C ε) ⦄ → {I : Set ℓ₃} → {O : I → Set ℓ₄} → Formula ε → program ε I O → I → Set ℓ₂
f ⊢ x ! i = f ⊢ (x i)

infix 25 _⊢_!_

⊢-decP : {ε : Effect {ℓ₁} {ℓ₂}} → ⦃ _ : Eq (Effect.C ε) ⦄ → {I : Set ℓ₃} → {O : I → Set ℓ₄} → (f : Formula ε) → (x : program ε I O) → (i : I) → Dec (f ⊢ x ! i)
does ⦃ ⊢-decP f x i ⦄ with ⊢-dec f (x i)
... | does because _ = does
proof ⦃ ⊢-decP f x i ⦄ with ⊢-dec f (x i)
... | _ because proof = proof

_▸_⊢_!_ : {ε : Effect {ℓ₁} {ℓ₂}} → ⦃ Eq (Effect.C ε) ⦄ → {I : Set ℓ₃} → {O : I → Set ℓ₄} → ℕ → Formula ε → recursiveProgram ε I O → I → Set ℓ₂
n ▸ f ⊢ x ! i = f ⊢ (recursionHandler x n) i

infix 25 _▸_⊢_!_

⊢-decR : {ε : Effect {ℓ₁} {ℓ₂}} → ⦃ _ : Eq (Effect.C ε) ⦄ → {I : Set ℓ₃} → {O : I → Set ℓ₄} → (n : ℕ) → (f : Formula ε) → (x : recursiveProgram ε I O) → (i : I) → Dec (n ▸ f ⊢ x ! i)
